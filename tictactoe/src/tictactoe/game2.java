...
public class game2 {
	...
	private static final long serialVersionUID = 5916872554131039979L;

	public static move findBestMove(char [][] b) {
		int bestVal = -1000;
	...
	public static int miniMax(char[][] b, int depth, Boolean isMaximizingPlayer) {
	...
	public static void main(String[] args) {
		...
		int moveCount = 0;
		while(evaluate(gameBoard) != 10 && evaluate(gameBoard) != -10) {
			...
			if(moveCount == 0) {
				...
				startingMove.row = 1;
				startingMove.col = 1;
				makeMove(startingMove, gameBoard, true);
			}
			else {
				move bestMove = findBestMove(gameBoard);
				makeMove(bestMove, gameBoard, true);
			}
			...
			playerMove.row = row;
			playerMove.col = col;
			makeMove(playerMove, gameBoard, false);
	...
	public static void makeMove(move playerMove, char[][] b, Boolean isMaximizingPlayer) {
		int row, col;
		row = playerMove.row;
		col = playerMove.col;
		if(b[row][col]== ' ') {
